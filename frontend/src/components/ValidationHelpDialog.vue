<template>
  <v-dialog v-model="storeState.helpModalVisibility" width="750">
    <v-card>
      ...
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn color="primary" text @click="closeValidationHelpDialog()">
          Close
        </v-btn>
      </v-card-actions>
    </v-card>
...
export default {
  ...
  methods: {
    closeValidationHelpDialog() {
      store.setHelpModalVisibility(false);
    }
  }
};
</script>
